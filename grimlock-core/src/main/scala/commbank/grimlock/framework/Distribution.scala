...
trait ApproximateDistribution[P <: HList, C <: Context[C]] { self: Matrix[P, C] =>
  ...
  def quantiles[
    ...
    ev4: ApproximateDistribution.QuantilesTuner[C#U, T]
  ): C#U[Cell[Q]]
  ...
  def countMapQuantiles[
    ...
    ev2: Position.NonEmptyConstraints[R],
    ev3: Position.GreaterThanConstraints[Q, S],
    ev4: ApproximateDistribution.CountMapQuantilesTuner[C#U, T]
  ): C#U[Cell[Q]]
  ...
  def tDigestQuantiles[
    ...
    ev2: Position.NonEmptyConstraints[R],
    ev3: Position.GreaterThanConstraints[Q, S],
    ev4: ApproximateDistribution.TDigestQuantilesTuner[C#U, T]
  ): C#U[Cell[Q]]
  ...
  def uniformQuantiles[
    ...
    ev2: Position.NonEmptyConstraints[R],
    ev3: Position.GreaterThanConstraints[Q, S],
    ev4: ApproximateDistribution.UniformQuantilesTuner[C#U, T]
  ): C#U[Cell[Q]]
}
...
private[grimlock] case class QuantileImpl[
  ...
  probs: List[Double],
  quantiser: Quantiles.Quantiser,
  position: Locate.FromPositionWithValue[S, Double, Q],
  nan: Boolean
) {
  ...
  def prepare(cell: Cell[P]): Double = cell.content.value.as[Double].getOrElse(Double.NaN)
  ...
  def present(pos: Position[S], out: O)(implicit ev: Value.Box[Double]): Option[Cell[Q]] =
    if (!out._2.isNaN || nan)
