...
/** Trait for matrix operations using a `RDD[Cell[P]]`. */
trait Matrix[
  L <: Nat,
  P <: Nat
  ...
  def slide[
    ...
    data
      ...
      .tunedStream(tuner, (key, itr) => Slide.stream[P, slice.S, slice.R, Q, window.type](window, ascending)(key, itr))
...
/** Trait for methods that reduce the number of dimensions or that can be filled using a `RDD[Cell[P]]`. */
...
/** Trait for methods that reshapes the number of dimension of a matrix using a `RDD[Cell[P]]`. */
